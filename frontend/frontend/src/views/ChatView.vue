...
const uniqById = (arr) => {
  const m = new Map()
  for (const it of normalizeList(arr)) {
    if (it && it.id != null) m.set(it.id, it)
  }
...
const selectedThread = computed(() => {
  if (selected.value?.type !== 'thread') return null
  return selected.value.payload || threads.value.find(t => t.id === selected.value.id) || null
})
const selectedGroup = computed(() => {
  if (selected.value?.type !== 'group') return null
  return selected.value.payload || groups.value.find(g => g.id === selected.value.id) || null
})
const selectedGroupOwner = computed(() => {
  const g = selectedGroup.value
  const ownerId = g?.owner?.id ?? g?.owner_id ?? g?.owner
  return ownerId != null && ownerId === auth.user?.id
})
const headerTitle = computed(() => {
  if (!selected.value) return 'Chat'
  if (selected.value.type === 'thread') return otherName(selectedThread.value)
  if (selected.value.type === 'group') return (selectedGroup.value?.name || 'Guruh')
  return 'Chat'
})
...
async function loadContacts() {
  ...
  try {
    ...
    contacts.value = normalizeList(data)
  } finally { contactsLoading.value = false }
}

function unwrapPage(data){
  // Supports both paginated and old array responses
  if (Array.isArray(data)) return { results: normalizeList(data), next: null }
  return { results: normalizeList(data?.results), next: data?.next || null }
}

function mergeMessages(existing, incoming){
  const map = new Map()
  for (const m of normalizeList(existing)) {
    if (m?.id != null) map.set(m.id, m)
  }
  for (const m of normalizeList(incoming)) {
    if (m?.id != null) map.set(m.id, m)
  }
...
async function loadThreads() {
  ...
  try {
    ...
    threads.value = normalizeList(data)
...
async function refreshAll() {
  ...
  if (selected.value?.type === 'thread') await loadThreadMessages(selected.value.id, { silent: true, reset: false })
  if (selected.value?.type === 'group') await loadGroupMessages(selected.value.id, { silent: true, reset: false })
}
...
async function loadThreadMessages(id, { silent = false, reset = false } = {}) {
  ...
  try {
    ...
    const incoming = normalizeList(page.results).slice().reverse() // oldest -> newest
...
async function loadGroupMessages(id, { silent = false, reset = false } = {}) {
  ...
  try {
    ...
    const incoming = normalizeList(page.results).slice().reverse()
    messages.value = reset ? incoming : mergeMessages(messages.value, incoming)
    await scrollBottom()
  } catch (e) {
    if (!silent) messagesErr.value = 'Messages olishda xatolik.'
...
async function send() {
  ...
  try {
    if (selected.value.type === 'thread') {
      await api.post(`/api/threads/${selected.value.id}/messages/`, { text })
      await loadThreadMessages(selected.value.id, { silent: true, reset: false })
      await loadThreads()
    } else if (selected.value.type === 'group') {
      await api.post(`/api/groups/${selected.value.id}/messages/`, { text })
      await loadGroupMessages(selected.value.id, { silent: true, reset: false })
      await loadGroups()
    }
    draft.value = ''
